...
> Slack OIDC strategy for Überauth.

## Installation

1. Setup your application at [Slack API](https://api.slack.com).

1. Add `:ueberauth_slack_oidc` to your list of dependencies in `mix.exs`:

    ```elixir
    def deps do
      [{:ueberauth_slack_oidc, "~> 1.0"}]
    end
    ```
...
1. Add Slack to your Überauth configuration:
    ...
    config :ueberauth, Ueberauth,
      providers: [
        slack: {Ueberauth.Strategy.SlackOIDC, []}
      ]
    ```
    ...
    config :ueberauth, Ueberauth,
      providers: [
        slack: {Ueberauth.Strategy.SlackOIDC, [team: "0ABCDEF"]}
      ]
    ```
